...
7. **국제 기준 채택** - Article 6, SBTi, ICVCM, Framework별 리스크
8. **공급 통로 다변화** - 탄소시장 생태계 구조
9. **이중계상 방지** - Corresponding Adjustments
...
- 상응조정(Corresponding Adjustment) 적용 의무
- 상응조정: 파리협정 제6조 하에서 국가 간 크레딧 이전 시 투명성 확보 메커니즘
- NDC 달성을 위한 국제 크레딧 사용 현황 공개

### 국가별 이중계상에 대한 입장 차이
- **짐바브웨, 파푸아 뉴기니, 케냐, 르완다 등**: 결정 이전까지 일시적 VCM 금지 또는 국외로 탄소크레딧 반출 금지
- **덴마크**: 자발적 목표 사용을 목적으로 국외로 탄소크레딧 반출 가능하나, 덴마크 NDC에 최종 감축 실적이 반영될것이란 커뮤니케이션 요구

## 10. CDP 질문 변경으로 본 마켓 트렌드 3: Baseline 허용 범위에 대한 입장 차이 대립
...
6. **슬라이드 번호 조정**: 총 22개 슬라이드로 확장

### 기술적 업데이트 (2025-07-02)
7. **한국어 현지화**: 모든 콘텐츠를 한국어로 번역 및 최적화
8. **매트릭스 다이어그램**: 탄소시장 분류 매트릭스 시각화 추가
9. **카테고리별 분포**: 6개 주요 카테고리별 통계 시각화
10. **CDP 테이블 개선**: 가독성 향상을 위한 스택형 레이아웃 적용
11. **슬라이드 재정렬**: 논리적 흐름 개선을 위한 순서 최적화

### 배포 및 보안 업데이트 (2025-07-02)
12. **PA-Deploy 구축**: GitHub Pages 자동 배포 시스템 구현
13. **보안 프로토콜**: 민감한 문서 보호를 위한 화이트리스트 방식 도입
14. **실시간 배포**: [carbonsteward.github.io/Carbon-Market-CDP](https://carbonsteward.github.io/Carbon-Market-CDP)
15. **랜딩 페이지**: 한국어 기업 디자인 표준 적용된 메인 페이지
16. **성능 최적화**: 단일 파일 배포로 빠른 로딩 및 오프라인 지원

### 도구 및 스크립트
17. **배포 상태 스크립트**: deploy-status.sh로 배포 모니터링
18. **보안 검증 도구**: pa-deploy-safe.sh로 민감 파일 차단
19. **GitHub Actions**: 자동화된 CI/CD 파이프라인 구축
20. **품질 게이트**: 보안 스캔 및 성능 검증 자동화

### 현재 상태
- **총 슬라이드**: 21개 (목차부터 이중계상 방지까지)
- **배포 상태**: ✅ 라이브 (GitHub Pages)
- **보안 수준**: 🔒 기업급 (민감 데이터 차단)
- **성능**: ⚡ 최적화 완료 (3초 이하 로딩)
